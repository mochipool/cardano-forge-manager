...
with patch('kubernetes.config.load_incluster_config'), \
     ...
     patch('cluster_manager.initialize_cluster_manager'):
    import forgemanager
...
class TestSocketBasedDetection(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.socket_patcher = patch.object(forgemanager, 'NODE_SOCKET', self.socket_path)
    ...
    def test_is_node_in_startup_phase_socket_ready(self):
        ...
        with patch('stat.S_ISSOCK', return_value=True), \
             patch('forgemanager.discover_cardano_node_pid', return_value=12345):
...
class TestCredentialManagement(unittest.TestCase):
    ...
    def test_ensure_secrets_leader_provision(self, mock_sighup):
        ...
        with patch.object(forgemanager, 'SOURCE_KES_KEY', self.source_kes), \
             patch.object(forgemanager, 'SOURCE_VRF_KEY', self.source_vrf), \
             patch.object(forgemanager, 'SOURCE_OP_CERT', self.source_cert), \
             patch.object(forgemanager, 'TARGET_KES_KEY', self.target_kes), \
             patch.object(forgemanager, 'TARGET_VRF_KEY', self.target_vrf), \
             patch.object(forgemanager, 'TARGET_OP_CERT', self.target_cert):
    ...
    def test_ensure_secrets_no_change_needed(self):
        ...
        for src, target in [(self.source_kes, self.target_kes),
...
class TestLeadershipElection(unittest.TestCase):
    ...
    def test_try_acquire_leader_success_vacant_lease(self, mock_cluster_manager):
        ...
        mock_cluster_manager.should_allow_local_leadership.return_value = (True, "allowed")
    ...
    def test_try_acquire_leader_blocked_by_cluster(self, mock_cluster_manager):
        ...
        mock_cluster_manager.should_allow_local_leadership.return_value = (False, "cluster_disabled")
        
        # Set initial state as leader
        forgemanager.current_leadership_state = True
        
        with patch('forgemanager.ensure_secrets') as mock_ensure, \
             patch('forgemanager.update_metrics') as mock_metrics:
    ...
    def test_try_acquire_leader_lease_conflict(self, mock_cluster_manager):
        ...
        self.mock_coord_api.patch_namespaced_lease.side_effect = ApiException(status=409)
    ...
    def test_create_lease_already_exists(self):
        ...
        mock_lease = self.create_mock_lease()
        self.mock_coord_api.create_namespaced_lease.side_effect = ApiException(status=409)
        self.mock_coord_api.read_namespaced_lease.return_value = mock_lease
...
class TestCRDManagement(unittest.TestCase):
    ...
    def test_update_leader_status_success(self, mock_cluster_manager):
        ...
        call_args = self.mock_custom_objects.patch_namespaced_custom_object_status.call_args
...
class TestEdgeCasesAndErrorHandling(unittest.TestCase):
    ...
    def test_startup_credentials_already_exist(self):
        ...
        try:
            ...
            with patch.object(forgemanager, 'SOURCE_KES_KEY', '/nonexistent/kes.skey'), \
                 patch.object(forgemanager, 'SOURCE_VRF_KEY', '/nonexistent/vrf.skey'), \
                 patch.object(forgemanager, 'SOURCE_OP_CERT', '/nonexistent/node.cert'), \
                 patch.object(forgemanager, 'TARGET_KES_KEY', target_kes), \
                 patch.object(forgemanager, 'TARGET_VRF_KEY', target_vrf), \
                 patch.object(forgemanager, 'TARGET_OP_CERT', target_cert):
    ...
    def test_file_comparison_exception_handling(self):
        ...
        try:
            ...
            with patch('forgemanager.os.stat', side_effect=PermissionError("Access denied")):
...
class TestIntegrationScenarios(unittest.TestCase):
    ...
    @patch('forgemanager.update_metrics')
    def test_complete_startup_sequence(self, mock_update_metrics, mock_start_metrics,
                                     mock_startup_cleanup, mock_provision_startup,
                                     mock_wait_socket, mock_cluster_manager):
        ...
        with patch('forgemanager.is_node_in_startup_phase', side_effect=[True, True, False]):
    ...
    def test_cluster_management_integration(self, mock_cluster_manager):
        ...
        mock_cluster_manager.should_allow_local_leadership.return_value = (True, "cluster_enabled")
